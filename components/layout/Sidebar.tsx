    ...
    return (
        <li className="my-1">
            <a
                ...
                onClick={handleClick}
                style={indentStyle}
                className={`flex items-center p-2 text-sm rounded-xl group relative overflow-hidden ${
                    isParentActive ? 'bg-primary text-white' :
            ...
            >
                <span className="z-10 flex items-center w-full">
                    {level === 0 && <span className={`mr-1 opacity-80 group-hover:opacity-100 ${isParentActive ? 'text-white' : ''}`}>{item.icon}</span>}
                    {level > 0 && <span className={`w-1.5 h-1.5 rounded-full ml-4 ${isActive || isParentActive ? 'bg-white' : 'bg-gray-400 group-hover:bg-gray-600'}`}></span>}
                    <span className={`${level > 0 ? '' : 'mr-3'} flex-1 truncate`}>{item.label}</span>
                    {hasChildren && <IconChevronLeft className={`w-4 h-4 ${isExpanded ? '-rotate-90' : ''} opacity-70`} />}
                </span>
            </a>
            {hasChildren && (
...
export const Sidebar: React.FC<SidebarProps> = ({ isOpen, activePage, onNavigate, onCloseMobile }) => {
    ...
    return (
        <>
            {/* Mobile Overlay */}
            <div 
                className={`fixed inset-0 bg-black/50 z-60 md:hidden ${isOpen ? 'block' : 'hidden'}`} 
                onClick={onCloseMobile}
            />

            {/* Sidebar Container - Removed Transitions */}
            <aside 
                className={`fixed top-0 right-0 h-full w-72 bg-white dark:bg-[#151923] border-l border-gray-200 dark:border-[#2d324a] z-70 ${isOpen ? 'block' : 'hidden md:hidden'}`}
            >
                <div className="flex items-center justify-center h-16 border-b dark:border-[#2d324a] bg-white dark:bg-[#151923]">
                ...
                <nav className="p-4 h-[calc(100vh-64px)] overflow-y-auto scrollbar-thin scrollbar-thumb-gray-300 dark:scrollbar-thumb-gray-600">
