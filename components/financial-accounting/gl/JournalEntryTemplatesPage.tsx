...
import { IconPlusCircle, IconEdit, IconTrash, IconFileText, IconCopy } from '../../Icons';
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex justify-between items-center">
                <div>
                    <h1 className="text-2xl font-bold text-gray-800 dark:text-white">کتابخانه الگوهای سند</h1>
                    <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">الگوهای آماده برای سرعت بخشیدن به ثبت اسناد تکراری.</p>
                </div>
                <button onClick={() => { setEditingTemplate(null); setModalOpen(true); }} className="btn-primary flex items-center gap-2 shadow-lg shadow-primary/20"><IconPlusCircle className="w-5 h-5"/> ایجاد الگوی جدید</button>
            </div>
            
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                {templates.map(t => (
                    <div key={t.id} className="group bg-white dark:bg-[#1E2130] rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 p-5 hover:shadow-lg hover:border-primary/30 transition-all duration-300 flex flex-col">
                        <div className="flex justify-between items-start mb-3">
                            <div className="p-3 rounded-xl bg-primary-50 dark:bg-gray-700 text-primary group-hover:bg-primary group-hover:text-white transition-colors">
                                <IconFileText className="w-6 h-6"/>
                            </div>
                            <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                <button onClick={() => handleEdit(t)} className="p-2 text-gray-400 hover:text-blue-500 hover:bg-blue-50 rounded-lg transition-colors"><IconEdit className="w-4 h-4"/></button>
                                <button onClick={() => deleteTemplate(t.id)} className="p-2 text-gray-400 hover:text-rose-500 hover:bg-rose-50 rounded-lg transition-colors"><IconTrash className="w-4 h-4"/></button>
                            </div>
                        </div>
                        
                        <h3 className="font-bold text-gray-800 dark:text-white text-lg mb-1">{t.name}</h3>
                        <p className="text-sm text-gray-500 line-clamp-2 mb-4 min-h-[2.5em]">{t.description || 'بدون توضیحات'}</p>
                        
                        <div className="mt-auto pt-4 border-t border-gray-100 dark:border-gray-700 flex items-center justify-between">
                            <span className="text-xs font-bold text-gray-400 bg-gray-100 dark:bg-gray-800 px-2 py-1 rounded">{t.lines.length} آرتیکل</span>
                            <button className="text-sm font-bold text-primary flex items-center gap-1 hover:underline">
                                استفاده <IconCopy className="w-3 h-3"/>
                            </button>
                        </div>
                    </div>
                ))}
                ...
                <button onClick={() => { setEditingTemplate(null); setModalOpen(true); }} className="rounded-2xl border-2 border-dashed border-gray-300 dark:border-gray-700 flex flex-col items-center justify-center p-6 text-gray-400 hover:text-primary hover:border-primary hover:bg-primary-50/50 transition-all min-h-[200px]">
                    <IconPlusCircle className="w-12 h-12 mb-3 opacity-50"/>
                    <span className="font-bold">ساخت الگوی جدید</span>
                </button>
            </div>
